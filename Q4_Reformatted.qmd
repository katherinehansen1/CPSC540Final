...
In this DAG, we visualize and explore the relationships between the variables `NumberStruckActual`, `FlightPhase`, `Altitude`, `ConditionsSky`, `Effect`, `Damage`, and `Origin`. Ultimately, we are aiming to quantify the direct effect of `FlightPhase` on `NumberStruckActual`, making `FlightPhase` our expose variable and `NumberStruckActual` our outcome variable. There exists a chain from `FlightPhase` -\> `Altitude` -\> `NumberStruckActual`, making `Altitude` a mediator between `FlightPhase` and `NumberStruckActual`. Because we are interested in the *direct* effect, we must control for `Altitude` in our model. To do so, we will include `Altitude` as an additional predictor in our GLM. Our GLM will be a simple linear regression using `FlightPhase` and `Altitude` to predict `NumberStruckActual`, as `NumberStruckActual` is a continuous variable.
...
dagitty(" dag { FlightPhase -\> NumberStruckActual ConditionsSky -\> NumberStruckActual Effect -\> NumberStruckActual Damage -\> NumberStruckActual OriginState -\> FlightPhase OriginState -\> ConditionsSky FlightPhase -\> Altitude FlightPhase -\> Damage Effect -\> ConditionsSky Origin -\> ConditionsSky Damage -\> ConditionsSky Effect -\> Damage }")
...
```{r}
freq_model <- glm(NumberStruckActual ~ FlightPhase + Altitude, 
                     data = data, 
...
To complete Bayesian parameter estimation, we used a prior of N(0, 2.5),). We also used a N(0, 2.5) for the intercept. These priors are weakly informative, as our prior knowledge of bird strikes is limited. We also selected a normal distribution, because we believe that it is most likely that these flight phases have a are centered around 0, because we think it is reasonable for any of these flight phases to have either a positive or negative effect on the `NumberStruckActual`. We used a log link function, as we want to ensure that all of our outcomes are positive.
